...
Std_ReturnType Crypto_ProcessJob(uint32 objectId, Crypto_JobType* job) {
    ...
    if (DriverState == ACTIVE)
    {
        if (job == currentJob)
        {
            ...
            else // UPDATE or FINISH
            {
                // Process next chunk or finalize (Note : in case of finish, Don't forget to go to idle state and to clear currentJob)
            }
        }
        else // job != currentJob
        {
            ...
            {
                // Another job called UPDATE or FINISH — reject
