...
static Std_ReturnType AesGcm_Start(driveobject_type* drv, const Crypto_JobType* job) {
    ...
    int ret = mbedtls_gcm_setkey(&mbedtls_gcm_context, MBEDTLS_CIPHER_ID_AES,
                                keyData, keyLength * 8);
    ...
    size_t iv_len = 12;
    ret = mbedtls_gcm_starts(&mbedtls_gcm_context, mode, iv, iv_len);
...
static Std_ReturnType AesGcm_Update(driveobject_type* drv, const Crypto_JobType* job) {
    ...
    size_t output_len;
    int ret = mbedtls_gcm_update(&mbedtls_gcm_context, input, input_len,
                                output, input_len, &output_len);
    return (ret == 0) ? E_OK : E_NOT_OK;
}
//
static Std_ReturnType AesGcm_Finish(driveobject_type* drv, Crypto_JobType* job) {
    ...
    if (job->jobPrimitiveInfo->primitiveInfo->service == CRYPTO_MACGENERATE) {
        // Generate MAC tag
        ret = mbedtls_gcm_finish(&mbedtls_gcm_context, tag, tag_len);
    } else {
//        // Verify MAC tag
        ret = mbedtls_gcm_finish(&mbedtls_gcm_context, tag, tag_len);
...
static void AesGcm_Reset(driveobject_type* drv) {
    mbedtls_gcm_free(&mbedtls_gcm_context);
}
...
Std_ReturnType Crypto_ProcessJob(uint32 objectId, Crypto_JobType* job)
{
    ...
    CryptoDriverObject*          drv   = &Crypto_GlobalConfigPtr->DriverObjectRefs[objectId]; //cfg cyptodriver
...
//    Crypto_JobStateType          state = ObjectsStatus[objectId];
    ...
    if (drv == &CryptoObject_MAC) {
        ...
        if (finish) {
            ...
            retVal =  AesGcm_Finish(drv, job);
            if (retVal == E_OK) {
                //ObjectsStatus[objectId] = CRYPTO_JOBSTATE_IDLE;
                obj->status = CRYPTO_JOBSTATE_IDLE;
                job->jobState = CRYPTO_JOBSTATE_IDLE;
                Crypto_ResetBuffers(drv);
            } else {
...
int main()
{
    ...
    macVerifyJob.PrimitiveInfo.primitiveInfo.algorithm.family = CRYPTO_ALGOFAMILY_CMAC; // Example: CMAC
    macVerifyJob.PrimitiveInfo.primitiveInfo.algorithm.keyLength = 128; // Key length in bits
    macVerifyJob.PrimitiveInfo.primitiveInfo.algorithm.mode = CRYPTO_ALGOMODE_CBC; // Example mode

    // Set cryptographic key ID (example value)
    macVerifyJob.cryptoKeyId = CRYPTO_KEY_ID_0; // Use your actual key ID
    ...
    if(*verifyPtr == CRYPTO_VERIFY_SUCCESS) {
