...
static Std_ReturnType AesGcm_Finish(driveobject_type* drv, Crypto_JobType* job) {
    ...
    if (job->jobPrimitiveInfo->primitiveInfo->service == CRYPTO_MACGENERATE) {
        ret = mbedtls_gcm_finish(&gcm,output,sizeof(output),(size_t*)job->jobPrimitiveInputOutput.outputLengthPtr, tag, tag_len); 
    ...
    else if (job->jobPrimitiveInfo->primitiveInfo->service == CRYPTO_MACVERIFY)
    {
        ...
        ret = mbedtls_gcm_finish(&gcm,output,sizeof(output),&job->jobPrimitiveInputOutput.outputLengthPtr, tag, tag_len);
