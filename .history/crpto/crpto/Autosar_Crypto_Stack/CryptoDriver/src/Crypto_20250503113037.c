...
static Std_ReturnType AesGcm_Start(driveobject_type* drv, const Crypto_JobType* job) {
    ...
    int mode;
    if(job->jobPrimitiveInfo->primitiveInfo->service == CRYPTO_MACGENERATE || job->jobPrimitiveInfo->primitiveInfo->service == CRYPTO_MACVERIFY)
        {
...
static Std_ReturnType AesGcm_Finish(driveobject_type* drv, Crypto_JobType* job) {
    ...
    else 
    {
        ...
        // size_t expacted_tag_len = job->jobPrimitiveInputOutput.secondaryInputLength;
        // printf("\nTag :");
...
int main()
{
    ...
    // MAC array (16 bytes)
    uint8_t mac[] = {
        0x18, 0xfb , 0x4b , 0x0f , 0x2a , 0xd0 , 0x25 , 0xbc , 0xe4 , 0x6b , 0x67 , 0x62 , 0x9f , 0x1b , 0xed , 0x9f 
