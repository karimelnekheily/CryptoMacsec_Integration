...
Std_ReturnType Crypto_ProcessJob(uint32 objectId, Crypto_JobType* job) {
    ...
    if (ObjectsStatus[objectId] == CRYPTO_JOBSTATE_ACTIVE)
    {
        if (job == currentJobs[objectId])
        {
            ...
            else if(job->jobPrimitiveInputOutput.mode == CRYPTO_OPERATIONMODE_UPDATE) //Update Or Finish(Don't forget to go to idle state and reset buffers)0x02 Error, 0x03 
            {
                canProcessJob = true;
                needsReset = false;
            }
