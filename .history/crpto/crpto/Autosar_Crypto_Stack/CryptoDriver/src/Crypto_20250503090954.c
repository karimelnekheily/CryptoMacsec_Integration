...
int main() {
    ...
    // 3. Process plaintext (could be called multiple times)
    size_t input_len = strlen((char *)plaintext);
    size_t output_len;
    ...
    if (ret = mbedtls_gcm_finish(
        &ctx,
        output,         // Output buffer for final data
        sizeof(output), // Size of output buffer
        &output_len,    // Actual length of final data
        tag,            // Tag buffer
        TAG_LEN         // Tag length
) != 0) {
    ...
    // 3. Process ciphertext (could be called multiple times)
    ...
    if ((ret = mbedtls_gcm_finish(&ctx, check_tag, TAG_LEN)) != 0) {
        printf("Finish failed: %d\n", ret);
        return 1;
    }
