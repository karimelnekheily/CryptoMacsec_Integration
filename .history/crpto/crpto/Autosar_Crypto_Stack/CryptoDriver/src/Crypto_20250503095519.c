...
Crypto_JobPrimitiveInfoType encryptJob = {
    .callbackId = 1,                              // Assume 1 maps to a callback in your callback table
...
static Std_ReturnType AesGcm_Finish(driveobject_type* drv, Crypto_JobType* job) {
    ...
    if (job->jobPrimitiveInfo->primitiveInfo->service == CRYPTO_MACGENERATE) {
        ...
        job->jobPrimitiveInputOutput.outputPtr = tag;
    ...
    {
//        // Verify MAC tag
        uint8* tag = job->jobPrimitiveInputOutput.secondaryInputPtr;
        size_t tag_len = job->jobPrimitiveInputOutput.secondaryInputLength;
        printf("\nTag :");
        for (size_t i = 0; i < sizeof(tag)/sizeof(tag[0]); i++) {
            printf("%02x ", tag[i]);
        }
...
int main()
{
    ...
    // MAC array (16 bytes)
    uint8_t mac[] = {
        0xbc, 0x4f , 0x08 , 0xed , 0x58 , 0x36 , 0x67 , 0xf3 
    ...
    printf("4\n");
    // if (macVerifyJob.jobPrimitiveInfo->primitiveInfo != NULL) {
    macVerifyJob.jobPrimitiveInfo = &encryptJob ;
    // }
    printf("5\n");
    printf("%d",macVerifyJob.jobPrimitiveInfo->primitiveInfo->service);
    ...
    output = macVerifyJob.jobPrimitiveInputOutput.outputPtr;
    // Check verification result
    
    if(macVerifyJob.jobPrimitiveInfo->primitiveInfo->service == CRYPTO_MACGENERATE)
    {
        printf("\nOutput Tag :");
        for (size_t i = 0; i < *(macVerifyJob.jobPrimitiveInputOutput.outputLengthPtr); i++) {
            printf("%02x ", macVerifyJob.jobPrimitiveInputOutput.outputPtr[i]);
        }
    }
...
//     // Buffers for encryption results
...
//     // 1. Set AES key
//     if ((ret = mbedtls_gcm_setkey(&ctx, MBEDTLS_CIPHER_ID_AES, 
//                                 key, AES_KEY_SIZE)) != 0) {
//         printf("Key setup failed: %d\n", ret);
//         return 1;
//     }

//     // 2. Initialize encryption operation
//     if ((ret = mbedtls_gcm_starts(&ctx, MBEDTLS_GCM_ENCRYPT,
//                                  iv, IV_LEN)) != 0) {
//         printf("GCM start failed: %d\n", ret);
//         return 1;
//     }

//     // 3. Process plaintext
//     size_t input_len = strlen((const char *)plaintext);
//     if ((ret = mbedtls_gcm_update(&ctx, plaintext, input_len,
...
//     // 4. Finalize encryption and get MAC tag
//     if ((ret = mbedtls_gcm_finish(&ctx, 
//                                 ciphertext + output_len,  // Final output (if any)
//                                 sizeof(ciphertext) - output_len,
//                                 &output_len,             // Final output length
//                                 tag,                     // Tag buffer
//                                 TAG_LEN)) != 0) {
...
//     printf("Ciphertext: ");
//     for (size_t i = 0; i < input_len; i++) printf("%02x ", ciphertext[i]);
//     printf("\nTag: ");
//     for (int i = 0; i < TAG_LEN; i++) printf("%02x ", tag[i]);
//     printf("\n\n");

//     /*********************
//      * Decryption + MAC Verification
//      *********************/
//     mbedtls_gcm_init(&ctx);
...
//     // 2. Initialize decryption with same IV
//     if ((ret = mbedtls_gcm_starts(&ctx, MBEDTLS_GCM_DECRYPT,
//                                  iv, IV_LEN)) != 0) {
//         printf("GCM start failed: %d\n", ret);
//         return 1;
//     }
...
//     if ((ret = mbedtls_gcm_update(&ctx, ciphertext, input_len,
...
//     // 4. Finalize and verify MAC
//     unsigned char check_tag[TAG_LEN];
//     if ((ret = mbedtls_gcm_finish(&ctx,
//                                 decrypted + output_len,   // Final output
//                                 sizeof(decrypted) - output_len,
//                                 &output_len,             // Final output length
//                                 check_tag,               // Tag buffer
//                                 TAG_LEN)) != 0) {
//         printf("Finish failed: %d\n", ret);
//         return 1;
//     }
...
//     // Verify MAC
//     if (memcmp(tag, check_tag, TAG_LEN) != 0) {
