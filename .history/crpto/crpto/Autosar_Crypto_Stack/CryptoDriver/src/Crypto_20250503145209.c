...
static Std_ReturnType AesGcm_Start(driveobject_type* drv, const Crypto_JobType* job) {
    ...
    else(job->jobPrimitiveInfo->primitiveInfo->service == CRYPTO_DECRYPT)
        {
...
int main()
{
    printf("Crypto Operation Sequence:\n");
    ...
    /* Single Crypto_JobType reused for each primitive */
    Crypto_JobType job;
    ...
    job.jobPrimitiveInputOutput.outputLengthPtr = &outputLength;

    job.jobPrimitiveInfo  = &encryptJob;
    job.cryptoKeyId       = 0;
    job.jobState          = CRYPTO_JOBSTATE_ACTIVE;
    ...
    printf("Encrypted Data (%u bytes): ", outputLength);
    ...
    uint32  encryptedLength = outputLength;
    memcpy(encryptedData, output, encryptedLength);

    /* -------------------- */
    /* 3. DECRYPT */
    /* -------------------- */
    job.jobPrimitiveInputOutput.inputPtr        = encryptedData;
    job.jobPrimitiveInputOutput.inputLength     = encryptedLength;
    job.jobPrimitiveInputOutput.outputPtr       = output;
    job.jobPrimitiveInputOutput.outputLengthPtr = &outputLength;

    job.jobPrimitiveInfo  = &decryptJob;
    job.cryptoKeyId       = 0;
    job.jobState          = CRYPTO_JOBSTATE_ACTIVE;
    ...
    printf("Decrypted Data (%u bytes): ", outputLength);
    for (uint32 i = 0; i < outputLength; i++) {
        printf("%02x ", output[i]);
    }
    printf("\n");
    ...
    job.jobPrimitiveInputOutput.secondaryInputLength = 0;
    job.jobPrimitiveInputOutput.verifyPtr            = NULL;

    job.jobPrimitiveInfo  = &macGenerateJob;
    job.cryptoKeyId       = 0;
    job.jobState          = CRYPTO_JOBSTATE_ACTIVE;
    ...
    printf("Generated MAC Tag (%u bytes): ", outputLength);
    for (uint32 i = 0; i < outputLength; i++) {
        printf("%02x ", output[i]);
    }
    printf("\n");

    /* Save MAC for verification */
    uint8_t generatedMac[32];
    uint32  macLength = outputLength;
    memcpy(generatedMac, output, macLength);
    ...
    job.jobPrimitiveInputOutput.inputPtr            = data;
    job.jobPrimitiveInputOutput.inputLength         = dataLength;
    job.jobPrimitiveInputOutput.secondaryInputPtr    = generatedMac;
    job.jobPrimitiveInputOutput.secondaryInputLength = macLength;
    job.jobPrimitiveInputOutput.verifyPtr            = &verifyResult;
    job.jobPrimitiveInputOutput.outputPtr           = NULL;
    job.jobPrimitiveInputOutput.outputLengthPtr     = NULL;

    job.jobPrimitiveInfo  = &verifyJob;
    job.cryptoKeyId       = 0;
    job.jobState          = CRYPTO_JOBSTATE_ACTIVE;
