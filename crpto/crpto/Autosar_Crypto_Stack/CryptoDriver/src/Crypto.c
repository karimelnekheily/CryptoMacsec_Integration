...
{
    if(job == NULL_PTR || objectId >= CRYPTO_MAX_DRIVER_OBJECTS) {
        return E_NOT_OK;
    }
    driveobject_type*  obj   =  &Driverobjects[objectId];   //Fetch the actual runtime object (driveobject_type) corresponding to the objectId.
    if (obj->status == CRYPTO_JOBSTATE_ACTIVE && obj->jobId == job->jobId) {
        obj->jobId = 0;
        obj->status = CRYPTO_JOBSTATE_IDLE; //Resets the driver state to IDLE.
        job->jobState = CRYPTO_JOBSTATE_IDLE; //Resets the object state to IDLE.
        AesGcm_Reset(obj);
        return E_OK;
    }
